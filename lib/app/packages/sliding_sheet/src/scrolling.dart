...
class _SlidingSheetScrollPosition extends ScrollPositionWithSingleContext {
  ...
  bool get isDismissible => sheet.widget.isDismissible && fromBottomSheet;
  ...
  bool get shouldMakeSheetNonDismissible =>
  ...
  // states such as when the sheet is not dismissible.
  double adjustDelta(double delta) {
    double result = delta;

    if (shouldMakeSheetNonDismissible) {
  ...
  Future<void> runScrollSimulation(double velocity,
    ...
    void _tick() {
      ...
      final shouldStopScrollOnBottomSheets = fromBottomSheet &&
          (currentExtent <= 0.0 || shouldMakeSheetNonDismissible);
      ...
      if (shouldStopOnUpFling || shouldStopOnDownFling) {
        ...
        super.goBallistic(shouldMakeSheetNonDismissible ? 0.0 : velocity);
